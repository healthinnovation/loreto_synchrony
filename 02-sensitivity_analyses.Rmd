...
# calculate mean and sd for each district
distr_stats <- data |>
  group_by(distr) |> 
  summarise(mean_cases_m = mean(cases_m),
            sd_cases_m = sd(cases_m),
            mean_cases_d = mean(cases_d),
            sd_cases_d = sd(cases_d))
...
for (i in 1:n) {
  ...
  by_distr <- data |>
    ...
    mutate(cases_m = rnorm(n(), mean = mean_cases_m, sd = sd_cases_m),
           cases_d = rnorm(n(), mean = mean_cases_d, sd = sd_cases_d)) |> 
